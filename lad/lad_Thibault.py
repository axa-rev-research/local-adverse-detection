...
class LocalSurrogate():
    ...
    '''
    FIX THIBAULT
    Pour compenser le fait que Papernot ait besoin des deux colonnes de predict
    '''
    def pred(self, x):
        return self.blackbox.predict(x)[:,1]
        

    def get_support_points(self, x_toexplain):

        n_points_ = self.n_support_points
        y_x_toexplain = self.pred([x_toexplain])[0]
    ...
    def get_segment_boundary_touchpoint(self, x_toexplain, support_point, segment_points, linear_model):
        ...
        segment_points_labels = self.pred(segment_points)
    ...
    def compute_surrogate(self, support_points, plot=False):
        ...
        y_surrogate = self.pred(X_surrogate)
